...
// Serialisation utils
function dobj_dump_delta (dobj, path, objvarname, precode, postcode) {
	const INDENT = "    ";
	local visited = [
	...
	function impl (appendable, val, indentationLevel, visited) {
		::Assert( ::isdeltacallable(appendable.append) );
		::assert_def( val );
		::assert_num( indentationLevel );
		::assert_ge( indentationLevel , 0 );

		local indent = ::bindfront(::assert_clb(appendable.append),
				::strmul(INDENT, indentationLevel));
		local write = ::bindfront(::foreachofargs,
				::successifier(::fcomposition(::assert_clb(appendable.append), ::assert_def)));
		...
		//     as both -- in fact, it is a subset of objects)
			...
			if ( not visited.isVisited(val) ) {
				...
				::list_foreach (val, [
					...
					@write: write,
					@indent: indent,
					@impl: ::bindback(::bindfront(impl, appendable), indentationLevel + 1, visited)
				]);
				indent(); write("})()");
			}
		}
		...
		// -- Delta Objects
			...
			if ( not visited.isVisited(objval) ) {
				...
				foreach (local key, ::dobj_keys(objval)) {
					...
					indent();
					write("{");
					impl(appendable, key, indentationLevel + 1, visited);
					write(": ");
					impl(appendable, objval[key], indentationLevel + 1, visited);
					write("}");
				}
...
//			impl(appendable, "cannot serialise methods: " + val, indentationLevel + 1, visited);
	...
	if (local result = ::toboolean(local fh = std::fileopen(path, "wt"))) {
		if (precode)
			std::filewrite(fh, precode);
		std::filewrite(fh, "\n\n// Autogenerated:\n", objvarname, " = ");
		impl([method append(str){std::filewrite(@fh,::assert_str(str));},@fh:fh], dobj, 1, visited);
		std::filewrite(fh, ";\n\n");
		if (postcode)
			std::filewrite(fh, postcode);
		std::fileclose(fh);
	}
	else
		::error().AddError("In dobj_dump_delta(): Could not open file for "
				"writing: ", path);
	return result;
}
