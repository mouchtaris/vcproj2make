...
function dobj_dump_delta (dobj, path, objvarname, precode, postcode) {
	const INDENT = "    ";
	function impl (appendable, val, indentationLevel) {
		...
		if ( ::isdeltastring(local strval = val) )
			...
			foreach (local key, ::dobj_keys(objval)) {
				...
				indent();
				write("{");
				impl(appendable, key, indentationLevel + 1);
				write(": ");
	...
	if (local result = ::toboolean(local fh = std::fileopen(path, "wt"))) {
		...
		std::filewrite(fh, "\n\n// Autogenerated:\n", objvarname, " = ");
		impl([method append(str){std::filewrite(@fh,::assert_str(str));},@fh:fh], dobj, 1);
		std::filewrite(fh, ";\n\n");
		if (postcode)
